...
let historyAdoptionsRepository: InMemoryAdoptionsRepository
let sut: GetHistoryAdoptionsUserUseCase

describe('History Adoptions User Use Case', () => {
  beforeEach(() => {
    historyAdoptionsRepository = new InMemoryAdoptionsRepository()
    sut = new GetHistoryAdoptionsUserUseCase(historyAdoptionsRepository)
  })
  it('should be able to get a history of user adoption', async () => {
    const userId = randomUUID()

    await historyAdoptionsRepository.create({
      id: randomUUID(),
      user_id: userId,
      pet_id: randomUUID(),
      user: {
